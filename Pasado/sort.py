...
# Use 'title' because we create that key in each task item.
...
def sort_alpha(dictionary):
    ...
    for key, value in dictionary.items():
        item = {
            "id": key,
            # Use 'value' to access task properties
            "title": value["text"],
            "ch": value["challenge"],
        }
        tasks.append(item)
...
def sort_todo_duedates(dictionary):
    ...
    # Sort tasks alphabetically by title
    sortedtasks = tasks.sort(key=lambda k: (k["date"][:10], k["createdAt"]), reverse=True)
