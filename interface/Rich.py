from utils.rich_utils import Table, Console, Panel, box, console
...
def print_stats(input_stats):
    ...
    if bool(input_stats.get("quest")) is True:
        user_stats.add_row(
            "[pink][b]:dragon:",
            f"""[pink]You're [i]in a quest.[/pink] Damage:[/] [up][b] 󰶣 {int(input_stats.get('quest').get('progress').get('up'))} [/][down][b] 󰶡 {int(input_stats.get('quest').get('progress').get('down'))}""",
        )
    ...
    about_panel.add_row(about, display_stats)
