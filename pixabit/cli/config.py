...
- Optional Tag IDs (Optional[str]): CHALLENGE_TAG_ID, PERSONAL_TAG_ID, etc.
...
loaded: bool = load_dotenv(dotenv_path=ENV_FILE_PATH, verbose=False, override=False)

if not loaded and ENV_FILE_PATH.exists():
    # This might happen if the .env file is empty or has syntax errors
    log.warning("[warning]⚠️ .env found but `load_dotenv` failed to load values (empty/malformed?).[/warning]")
elif not ENV_FILE_PATH.exists():  # Should not happen if check_env_file worked
    log.error(f"[error]❌ CRITICAL: .env file still not found at [file]{ENV_FILE_PATH}[/]. Exiting.[/error]")
    sys.exit(1)
...
if not HABITICA_USER_ID or not HABITICA_API_TOKEN:
    log.error("\n" + "=" * 60, style="error")
    log.error(" [error]❌ FATAL ERROR: Essential Configuration Missing![/error]")
    if not HABITICA_USER_ID:
        log.error(" - `HABITICA_USER_ID` not found or empty in environment/.env.")
    if not HABITICA_API_TOKEN:
        log.error(" - `HABITICA_API_TOKEN` not found or empty in environment/.env.")
...
CHALLENGE_TAG_ID: Optional[str] = os.getenv("CHALLENGE_TAG_ID", "").strip() or None
PERSONAL_TAG_ID: Optional[str] = os.getenv("PERSONAL_TAG_ID", "").strip() or None
LEGACY_TAG_ID: Optional[str] = os.getenv("LEGACY_TAG_ID", "").strip() or None
PSN_TAG_ID: Optional[str] = os.getenv("PSN_TAG_ID", "").strip() or None
NOT_PSN_TAG_ID: Optional[str] = os.getenv("NOT_PSN_TAG_ID", "").strip() or None
NO_ATTR_TAG_ID: Optional[str] = os.getenv("NO_ATTR_TAG_ID", "").strip() or None
ATTR_TAG_STR_ID: Optional[str] = os.getenv("ATTR_TAG_STR_ID", "").strip() or None
ATTR_TAG_INT_ID: Optional[str] = os.getenv("ATTR_TAG_INT_ID", "").strip() or None
ATTR_TAG_CON_ID: Optional[str] = os.getenv("ATTR_TAG_CON_ID", "").strip() or None
ATTR_TAG_PER_ID: Optional[str] = os.getenv("ATTR_TAG_PER_ID", "").strip() or None
